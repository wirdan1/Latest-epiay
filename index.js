;(async () => {
  ...
  function requestGuard(req, res, next) {
    ...
    // simple per-window rate limiting by IP (keeps server healthy, still 100% free)
    const currentWindow = Math.floor(now.getTime() / API_RATE_LIMIT_WINDOW)
    const rateKey = `${ip}:${currentWindow}`

    if (!rateLimit.has(rateKey)) {
      rateLimit.set(rateKey, { count: 0, lastReset: now.getTime() })
    }
    ...
    // Optional: track monthly usage per IP (no hard block here, just bookkeeping)
    const currentMonth = now.getMonth()
    const usageKey = `${currentMonth}:${ip}`
    if (!apiUsage.has(usageKey)) {
      apiUsage.set(usageKey, { count: 0, resetMonth: currentMonth })
    }
  ...
  let scraper // Declare the scraper variable
  global.scraper = scraper = new Scraper("./lib/scrape_file")
      ...
      if (stats.isDirectory()) {
        ...
        try {
          ...
          if (module && module.run && typeof module.run === "function") {
            ...
            app.all(endpointPath, requestGuard, module.run) // use requestGuard instead of validateApiKey for all dynamic endpoints (no apikey needed)
            ...
            endpointObj[module.name || endpointName] = {
              ...
              requiresApiKey: false, // mark that endpoints no longer require an API key
            }
  ...
  app.get("/status", (req, res) => {
    ...
    res.json({
      status: true,
      message: "Free mode enabled. No API key required.",
      rateLimit: `${API_RATE_LIMIT} requests per ${API_RATE_LIMIT_WINDOW / 1000}s window (per IP)`,
      trackedMonth: now.getMonth(),
    })
  })
